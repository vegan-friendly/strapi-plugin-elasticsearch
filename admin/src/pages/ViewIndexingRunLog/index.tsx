...
const loadRecentIndexingRuns = (): Promise<any[]> => {
  return axiosInstance
    .get(apiFetchRecentIndexingRunLog)
    .then((resp) => resp.data)
    .then((data) => {
      return data;
    });
...
const ViewIndexingRunLog = () => {
  const [logTable, setLogTable] = useState<any[] | null>(null);
  useEffect(() => {
    loadRecentIndexingRuns().then(setLogTable);
  }, []);

  if (logTable === null) return <LoadingIndicatorPage />;
  else
    return (
