...
export default ({ strapi }) => {
  ...
  const importContentConfig = async (ctx) => {
    ...
    try {
      if (body['data']) {
        ...
        return updatedConfig;
      } else ctx.throw(400, 'Invalid parameters');
    } catch (err) {
      ctx.throw(500, err);
    }
  };
  ...
  const getCollectionConfig = async (ctx) => {
    if (ctx.params.collectionname) return configureIndexingService.getCollectionConfig({ collectionName: ctx.params.collectionname });
    else return null;
  };
